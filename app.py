...
request_params = {'token': 'xObtimAtKaOuH8LLgoYNICyntK0Y7hMhiBfrbFkk'}


def insert_msg():
    response = requests.get('https://api.groupme.com/v3/groups/	50699469/messages', params=request_params)
    ...
    for input_message in hasil_select:
        ...
        if pesan == input_message_db:
            ...
            print(message['id'])
            sql = "insert into tb_inbox values (null,'%s','%s','%s','1')" % (
            message['id'], input_message_db, time.strftime("%Y-%m-%d %H:%M:%S"))
            print(sql)
            cursor.execute(sql)
            db.commit()
        ...
        if success > 0:
            ...
            post_params = {'bot_id': '563b6b1d6962dfa4e078968881', 'text': to_send}
